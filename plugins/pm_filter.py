...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("Back ◀️", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"Page 🗓 {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages")]
        )
    elif off_set is None:
        btn.append([InlineKeyboardButton(f"Page 🗓 {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages"), InlineKeyboardButton("Next ▶️", callback_data=f"next_{req}_{key}_{n_offset}")])
    else:
        btn.append(
            [
...
async def advantage_spoll_choker(bot, query):
    ...
    if k==False:
        ...
        if files:
            ...
            k = await query.message.edit('⚠️️ 𝚃𝙷𝙸𝚂 𝙼𝙾𝚅𝙸𝙴 I𝚂 𝙽𝙾𝚃 𝚈𝙴𝚃 𝚁𝙴𝙻𝙴𝙰𝚂𝙴𝙳 𝙾𝚁 𝙰𝙳𝙳𝙴𝙳 𝚃𝙾 𝙳𝙰𝚃𝚂𝙱𝙰𝚂𝙴 ⚠️️')
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🌺 JOIN OUR CHANNEL 🌺', url='https://t.me/Cinema_World_Sri_Lanka')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)      
        await query.message.edit_text(
        ...
        buttons = [[
            InlineKeyboardButton('Back ◀️', callback_data='help')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            InlineKeyboardButton('Back ◀️', callback_data='manuelfilter')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            InlineKeyboardButton('Back ◀️', callback_data='help'),
            InlineKeyboardButton('Admin Mods', callback_data='admin')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            InlineKeyboardButton('Back ◀️', callback_data='extra')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"Page 1/{round(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="𝗡𝗲𝘅𝘁 ➡️",callback_data=f"next_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
